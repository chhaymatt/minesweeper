	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			while (!minesweeper.getDone()) {
				System.out.println("");
				System.out.print(String.format(ConsoleColours.CYAN + "Enter x coordinate (%s-%s inclusive): ", 1, size)
						+ ConsoleColours.RESET);
				int x = getIntBetweenBounds(0, size + 1, scanner);

				System.out.print(String.format(ConsoleColours.CYAN + "Enter y coordinate (%s-%s inclusive): ", 1, size)
						+ ConsoleColours.RESET);
			...
			if (playAgainInput == 1) {
				...
				System.out.println(ConsoleColours.GREEN + "Thank you for playing! You may close the console."
						+ ConsoleColours.RESET);
				break;
			}
		}
		...
		while (correctInput == false) {
			try {
				...
				if (output > lower && output < upper) {
					...
					System.out.println(ConsoleColours.YELLOW + "Enter a number between " + lower + " and " + upper
							+ ConsoleColours.RESET);
				}
